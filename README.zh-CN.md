...
- `/rebuttal_revise`：对已有的 existing rebuttal 做润色和收紧

## 它是什么

AutoRebuttal 不是一个单独拷贝出去的 skill 文件夹，而是一个可安装、可调用、可测试的 rebuttal package。
...
当作者把 paper 和 reviews 交给 agent 时，AutoRebuttal 不会直接跳到最终文字，而是先组织 review concerns，再决定格式和预算，最后才生成 rebuttal。
...
默认目标路径是 `~/.agents/skills/auto-rebuttal`。更多说明见 [`.codex/INSTALL.md`](.codex/INSTALL.md)。
...
- [`skills/auto-rebuttal/scripts/format_rebuttal_blocks.py`](skills/auto-rebuttal/scripts/format_rebuttal_blocks.py)

...
PACKAGE_NAME = "super-rebuttal"
CLAUDE_MARKETPLACE = "super-rebuttal-dev"
CLAUDE_REPOSITORY = "YoujunZhao/SuperRebuttal"
...
def resolve_home_path(home: str | pathlib.Path | None = None) -> pathlib.Path:
    ...
    override = os.environ.get("SUPERREBUTTAL_HOME")
    if override:
...
def run_codex_action(args: argparse.Namespace) -> int:
    ...
    if action == "install":
        ...
        print(f"Installed SuperRebuttal for Codex at {destination}")
        return 0
    if action == "update":
        destination = codex_update(home=args.home)
        print(f"Updated SuperRebuttal for Codex at {destination}")
        return 0
    if action == "remove":
        destination = codex_remove(home=args.home)
        print(f"Removed SuperRebuttal for Codex from {destination}")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Manage SuperRebuttal installation surfaces for Codex and Claude Code."
    )
    ...
    for action, help_text in (
        ("install", "Install SuperRebuttal into the Codex user-home skill path."),
        ("update", "Update SuperRebuttal in the Codex user-home skill path."),
        ("remove", "Remove SuperRebuttal from the Codex user-home skill path."),
    ):
        action_parser = codex_actions.add_parser(
            action,
            help=help_text,
            description=f"Codex {action} command for SuperRebuttal.",
        )
    ...
    for action, help_text in (
        ...
        action_parser = claude_actions.add_parser(
            action,
            help=help_text,
            description=f"Claude Code {action} command renderer for SuperRebuttal.",
        )
        action_parser.add_argument(
            "--print-only",
            action="store_true",
